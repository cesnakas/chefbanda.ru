...
<?if(!empty($_GET["act"])){
	...
	if (CModule::IncludeModule("catalog") && CModule::IncludeModule("sale") && CModule::IncludeModule("dw.deluxe")){
		...
		elseif($_GET["act"] == "selectSku"){
			if(!empty($_GET["params"]) &&
			   ...
			   !empty($_GET["props"])
				...
				if(!empty($_GET["product-change-full-prop"]) && $_GET["product-change-full-prop"] != "undefined"){
					ob_start();
					$APPLICATION->IncludeComponent(
						"dresscode:catalog.properties.list",
						"group",
				...
				$arFilter = Array("IBLOCK_ID"=>26, "ACTIVE"=>"Y", "PROPERTY_W_TSVET" => $arLastOffer["PRODUCT"]['PROPERTIES']['TSVET']['VALUE']);
				...
				$ipropValues = new \Bitrix\Iblock\InheritedProperty\ElementValues(23, intval($_GET["product_id"]));
				$seo_values  = $ipropValues->getValues();
				$arLastOffer["PRODUCT"]['SEO'] = $seo_values['ELEMENT_PREVIEW_PICTURE_FILE_TITLE']." ".$color;
				//$arLastOffer["PRODUCT"]['SEO'] = $_GET["iblock_id"];
				$arLastOffer["PRODUCT"]['TEST'] = $arLastOffer["PRODUCT"];
		...
		elseif($_GET["act"] == "addCart"){
			...
			if(empty($_GET["multi"])){
				$suitItems=SkuManager::prepareSuitToBasket($_GET['id']);
				if($suitItems){
					 
					$_GET["multi"]="1";
					$arIds=array();
					$arQuantity=array();
					if(!intval($_GET["q"]))$_GET["q"]=1;
					foreach($suitItems as $item){
						$arIds[]=$item['ID'];
						$arQuantity[]=$item['ID'].":".($item['QUANTITY']*$_GET["q"]);	
					}
					 
					$_GET["id"]=implode(";",$arIds);
					$_GET["q"]=implode(";",$arQuantity);
				}
			...
			if(!empty($_GET["multi"]) && !empty($_GET['id'])){
				...
				$ipropValues = new \Bitrix\Iblock\InheritedProperty\ElementValues($mxResult["IBLOCK_ID"], $mxResult['ID']);
				$arResult['SEO']  = $ipropValues->getValues();
				$arResult['NAME']=$arResult['SEO']['ELEMENT_PAGE_TITLE']?$arResult['SEO']['ELEMENT_PAGE_TITLE']:$arResult['NAME'];
	...
	false
			...
			if(!empty($_GET["id"])){
				...
				while($arRes = $res->GetNextElement()){ 
					$arResult["PRODUCT"] = $arRes->GetFields();
					$arResult["PRODUCT"]["PROPERTIES"] = $arRes->GetProperties();
					if(Bitrix\Main\Loader::includeModule('h2o.giftsert')){
						$arResult["PRODUCT"]['IS_SERT'] = h2o\Giftsert\GiftsertTable::isGiftSert($arResult["PRODUCT"]['ID']);
					}
					...
					$arResult["PRODUCT"]['YM_PRICE'] = $arTmpPrice["DISCOUNT_PRICE"];

					$mxResult = CCatalogSKU::GetProductInfo($arResult["PRODUCT"]["ID"]);

					$resB = CIBlockElement::GetByID($mxResult['ID']);
					if($ar_res = $resB->GetNext()){
						$arFilter = array('IBLOCK_ID' => $ar_res["IBLOCK_ID"], "=ID" => $ar_res['IBLOCK_SECTION_ID']);
						$rsSect = CIBlockSection::GetList(array('left_margin' => 'asc'),$arFilter);
						if ($arSect = $rsSect->GetNext())
						{
							$arResult["PRODUCT"]['SECTION_NAME'] = $arSect['NAME'];
						}
			
						$db_props = CIBlockElement::GetProperty($ar_res["IBLOCK_ID"], $mxResult['ID'], array("sort" => "asc"), Array("CODE"=>"PROIZVODITEL"));
						if($ar_props = $db_props->Fetch()){
							$arResult["PRODUCT"]['BRAND'] = $ar_props['VALUE_ENUM'];
						}
					}
				...
				if(!empty($_GET["phone"]) && !empty($_GET["id"])){
					...
					if(CModule::IncludeModule("iblock") && CModule::IncludeModule("sale") && CModule::IncludeModule("catalog")){
						...
						$isGiftSert = $_GET['isSert']=='Y' ? true : false;
						...
						if(!empty($arElement)){
							...
							if($isGiftSert){
								$sertMail = [];
								$sertMail['USER_EMAIL'] = $address;
								$sertMail['USER_NAME'] = $name;
								$sertMail['EMAIL'] = $email;
								$sertMail['NOMINAL'] = $arElement["~PRICE"];
								$sertMail['TEXT_CONGRATULATIONS'] = htmlspecialchars($_GET['message']);
								$sertMail['SENDER'] = "С Уважением, <br/>".$name.", <br/>".htmlspecialchars($_GET['email']);
								$sertMail['COUPON'] = null;
								$discount_id = null;
								switch($arElement["~PRICE"]){
									case "1000" : $discount_id=7; break;
									case "2000" : $discount_id=12; break;
									case "3000" : $discount_id=13; break;
									case "5000" : $discount_id=9; break;
									case "10000" : $discount_id=14; break;
								}
								$coupon = \Bitrix\Sale\Internals\DiscountCouponTable::generateCoupon(true);
								   ...
								   if ($addDb->isSuccess()) {
									  $sertMail['COUPON'] = $coupon;
								   }

								require_once "dompdf-tagged-pdf/dompdf_config.inc.php";

								$dompdf = new Dompdf\Dompdf();

								$html = '<html><head><style>p { font-family: DejaVu Sans }</style></head><body>
										<div style="width:765px; height:482px; text-align:right; margin:0 auto; display:block; background:url(/upload/h2o.giftsert/d52/d5296607ec1b046e0f6c5ef92efb76b8.png) 0 0 repeat">
									<span style="display: block; color: rgb(43, 42, 40); font-family: arial,sans-serif; font-size: 70px; padding: 305px 320px 0px 0px;">'.$sertMail['NOMINAL'].'</span>
								<span style="font-family: arial,sans-serif; display: block; color: rgb(0, 0, 0); text-transform: uppercase; font-size: 38px; padding: 0 80px 0 0;">'.$sertMail['COUPON'].'</span>
								</div><p>Для оплаты Вашего заказа подарочной картой, введите код сертификата в поле «Промокод» или «Применить купон» при оформлении заказа на сайте chefworks.ru, или продиктуйте менеджеру Интернет-магазина при оформлении заказа по телефону. Подробную информацию вы так же можете получить по телефону 8(800)555-03-90 (бесплатный звонок из регионов).</p>
								</body></html>';
								...
								$makeFile=CFile::MakeFileArray($_SERVER['DOCUMENT_ROOT']."/file.pdf");
								$arIMAGE = array_merge($makeFile, array("MODULE_ID"=>"main"));
								$fid = CFile::SaveFile($arIMAGE, "main");
								CEvent::Send("H2O_GIFTSERT_TO_GETTER", htmlspecialcharsbx($_GET["SITE_ID"]), $sertMail, "N", "", array($fid));
								CEvent::Send("H2O_GIFTSERT", htmlspecialcharsbx($_GET["SITE_ID"]), $sertMail, "N", "", array($fid));
							}
								...
								$USER_ID = intval($USER->GetID());
		  						...
		  						if($USER_ID == 0){
									...
									if(!empty($arUser)){
										...
										$USER->Register($email, $name, "", $newPass, $newPass, $email);
										$USER_ID = $USER->GetID();
									...
									if ($ar_dtype = $db_dtype->Fetch()){
									   ...
									   $item->setFields([
										...
										while ($props = $db_props->Fetch()){
											...
											}else if(strtoupper($props["CODE"]) == "EMAIL"){
												$arOrderProps[$props["ID"]] = BX_UTF != 1 ? iconv("UTF-8","windows-1251//IGNORE", htmlspecialcharsbx($_GET["phone"])) : htmlspecialcharsbx($email);
											}else if(strtoupper($props["CODE"]) == "ADDRESS"){
												$arOrderProps[$props["ID"]] = BX_UTF != 1 ? iconv("UTF-8","windows-1251//IGNORE", htmlspecialcharsbx($_GET["address1"])) : htmlspecialcharsbx($address);
											}
										}

										$arOrderProps[30] = BX_UTF != 1 ? iconv("UTF-8","windows-1251//IGNORE", htmlspecialcharsbx($_GET["lname"])) : htmlspecialcharsbx($_GET["lname"]);

										//FIO
										$arOrderProps[1] = $arOrderProps[29];
										if (!empty($arOrderProps[30])) {
											$arOrderProps[1] .= ' ';
											$arOrderProps[1] .= $arOrderProps[30];
										}
										...
										if(empty($arErrors)){
											...
											$arOrderFields = array(
											   ...
											   "USER_ID" => !empty($USER_ID) ? $USER_ID : IntVal($USER->GetID()),
											   "PAY_SYSTEM_ID" => $_GET['isSert']=="Y" ? 12 : $ptype["ID"],
											   "PRICE_DELIVERY" => 0,
											   "DELIVERY_ID" => 1,
											   "DISCOUNT_VALUE" => 0,
											   "TAX_VALUE" => 0.0,
...
function getLastOffer($arLastFilter, $arProps, $productID, $opCurrency, $enableMorePictures = false, $arPrices = array()){
	...
	if(!$rsLastOffer->SelectedRowsCount()){
		...
		if($obReturnOffer = $rsLastOffer->GetNextElement()){
			...
			$suitPrice=SkuManager::getSuitPrice($productFilelds['ID']);
			if($suitPrice){
				$productFilelds['PRICE']['PRICE']['PRICE']=$suitPrice;
				$productFilelds['PRICE']["DISCOUNT_PRICE"]=$suitPrice;
			}
